...
class TrainValTestSplitter:
    def __init__(self, path_to_data, path_to_labels, val=False):
        """
    ...
    def _split_data(self):
        ...
        data_train, data_test, labels_train, labels_test = train_test_split(self.data['path'], self.data['labels'], test_size=0.3,
                                                                            random_state=42)
        if self.val:
            data_train, data_val, labels_train, labels_val = train_test_split(data_train, labels_train, test_size=0.3, random_state=42)
